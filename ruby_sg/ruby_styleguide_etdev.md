# Etdev Custom Ruby Style Guide

This is my personal version of the [Ruby Style Guide](https://github.com/bbatsov/ruby-style-guide), where I've removed things that I feel I've already internalized so that I can focus on the things I still need to learn.  I'll continue to delete things as I get more comfortable with the language, so I wouldn't suggest using this document as a reference.

## Source Code Layout

* <a name="multiple-puts"></a>
  Print multiple things on one line with ``puts``
...
* <a name="single-line-classes"></a>
  Prefer a single-line format for class definitions with no body (``Class.new(ParentClass)`` makes a new class that inherits from ParentClass)
...
* <a name="no-single-line-methods"></a>
  Only use one-line method definitions for empty-body methods.
...
* <a name="spaces-operators"></a>
  Don't use spaces around the ``**`` (exponent) operator, but use them with
  all other mathematical etc. operators
...
* <a name="consistent-multi-line-chains"></a>
  ...
  good - leading `.` (Option A) and trailing `.` (Option B). I like **Option A**.
...
* <a name="no-trailing-array-commas"></a>
  ...
  when the items are not on separate lines. [Thoughtbot disagrees](https://github.com/thoughtbot/guides/tree/master/style/ruby)
...
* <a name="unless-for-negatives"></a>
  Favor `unless` OR the ``||`` operator over `if` for negative conditions.
  `some_condition || do_something` does something unless some_condition
...
* <a name="multiple-attr-readers"></a>
  When you have multiple attr_readers, writers, accessors etc.
...
* <a name="heredocs"></a>
  Use ``heredocs`` for really long multi-line strings
  ```Ruby
  # bad
  puts "This is a really long line of text that\n \
  takes up more than eighty characters\n \
  and is kind of annoying to write out with\n \
  backslashes and stuff."

  # good
  puts <<-EOT
    This is another really long line of text
    that takes up more than eighty characters
    but when it's in a heredoc, we don't need
    to add newline characters etc.
  EOT
  ```
